    ...
    <div class="main">
      <div class="left">
        <div class="row">
          ...
          <div @click="switchType()" class="search-button" v-focusable>
        ...
        <div class="options">
          <div class="item" v-focusable>
            <svg width="24" height="24" stroke-width="1.5" viewBox="0 0 24 24" fill="none"
              ...
              <path
                  ...
                  stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M6 21H7C8.10457 21 9 20.1046 9 19V16H6C4.89543 16 4 16.8954 4 18V19C4 20.1046 4.89543 21 6 21Z"
                    stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            单曲
          </div>
          <div class="item" v-focusable>
            <svg width="24" height="24" stroke-width="1.5" viewBox="0 0 24 24" fill="none"
                 xmlns="http://www.w3.org/2000/svg">
              <path d="M15 2.19995C19.5645 3.12649 23 7.162 23 11.9999C23 16.8378 19.5645 20.8733 15 21.7999"
                    stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M15 9C16.1411 9.28364 17 10.519 17 12C17 13.481 16.1411 14.7164 15 15" stroke="currentColor"
          ...
          <div class="item" v-focusable>
            <svg width="24" height="24" stroke-width="1.5" viewBox="0 0 24 24" fill="none"
                 xmlns="http://www.w3.org/2000/svg">
              <path d="M1 20V19C1 15.134 4.13401 12 8 12V12C11.866 12 15 15.134 15 19V20" stroke="currentColor"
                    stroke-linecap="round"/>
              <path d="M13 14V14C13 11.2386 15.2386 9 18 9V9C20.7614 9 23 11.2386 23 14V14.5" stroke="currentColor"
                    stroke-linecap="round"/>
              <path
      ...
      <div class="right">
        <div v-focusable class="item" v-for="(item,index) in searchResult" :key="index">
          <div class="name"> {{ item.name }} </div>
          <div class="author"> {{ item.ar[0].alias[0] }} </div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async switchType() {
      try {
        let search = await this.$axios.get('/cloudsearch', {
          params: {
        ...
        if (this.currentType === 1) {
          console.log(search);
          this.searchResult = search.data.result.songs;
        }
      } catch (error) {
        console.log(error);
      }
